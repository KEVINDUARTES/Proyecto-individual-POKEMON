...
const { Pokemon, Type } = require("../db"); //importo de bd

// GET A LOS DATOS DE LA API 
const getApiInfo = async () => {
    const resp = await axios
      .get("https://pokeapi.co/api/v2/pokemon?limit=40")//pido info a la api por get
      .then((data) => {//lee la data
        return data.data.results; //retorna la data
      })
      .then((data) => {
        return Promise.all(data.map((res) => axios.get(res.url))); // ENTRAR A CADA ELEMENTO Y HACERLE UN GET A SU URL y SACARLE INFO
      })
      .then((data) => {
...
//CONCATENACION DE LOS DOS RESULTADOS ENCONTRADOS..
  ...
  module.exports = getAllPokemons;//exporto a pokemons